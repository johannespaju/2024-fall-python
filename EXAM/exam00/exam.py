...
def tic_tac_toe(game: list) -> int:
    ...
    for column in range(3):
        if game[0][column] == game[1][column] == game[2][column] != 0:
...
def rainbows(field: str, lower=False) -> int:
    ...
    """
    if len(field) >= 7 and field[:7].lower() == "rainbow" or field[:7][::-1].lower() == "rainbow":
        return 1 + rainbows(field[7:])
    if len(field) >= 7:
        return 0 + rainbows(field[1:])
    return 0
...
def longest_substring(text: str) -> str:
    ...
    for i, char in enumerate(text):
        ...
        if lower_char in seen and seen.get(lower_char) >= start:
            start = seen[lower_char] + 1
...
def create_student(name: str, grades: list, credit_points: int) -> Student:
    ...
    return Student(name, round((sum(grades) / len(grades)), 3), credit_points)
...
def get_top_student_with_credit_points(students: list, min_credit_points: int):
    ...
    """
    eligible_students = [s for s in students if s.credit_points >= min_credit_points]
    if not eligible_students:
        return None
    return max(eligible_students, key=lambda s: s.average_grade)
...
def add_result_to_student(student: Student, grades_count: int, new_grade: int, credit_points) -> Student:
    ...
    """
    grade_sum = grades_count * student.average_grade
    grades_count += 1
    grade_sum += new_grade

    student.average_grade = round(grade_sum / grades_count, 3)
...
class Hotel:
    ...
    def book_room(self, required_features: list) -> Optional[Room]:
        ...
        for room in self.rooms:
            if not room.booked:
                matching_features = len(set(required_features) & set(room.get_features()))
                if (
                        matching_features > max_matches or
                        (matching_features == max_matches and room.get_number() < (best_match.get_number() if best_match else 0))
                ):
                    best_match = room
                    max_matches = matching_features
    ...
    def get_most_profitable_feature(self) -> Optional[str]:
        ...
        return min(
            (feature for feature in feature_profits),
            key=lambda x: (-feature_profits[x], x)
        )
